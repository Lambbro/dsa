...
struct MedicalRecord : MedicalRecordBase {
        ...
        for (int i=0;i<n;i++) {
            ...
            do {
                ...
                if (search_stack(sm, med.id)) {
                    ...
                    med.price = med.price * (1-discount/100);
                    break;
                }
                cout << string(TAB_WIDTH, ' ') << "Xin moi nhap lai!!!" << endl;
            } while (true);
    ...
    void output() {
        ...
        cout << string(TAB_WIDTH, ' ') << "Giam gia: " << discount << endl;
...
//Exam
void fix_discount (StackMedicalRecord &smr, StackPatient sp, StackMedicine sm) {
    string id;
    int chon_hs;
    cin.ignore();
    cout << string(TAB_WIDTH, ' ') << "Nhap id hsba can sua: "; getline(cin, id);
    ...
    if (p==NULL) {
        ...
        float dis; cout << string(TAB_WIDTH, ' ') << "Giam gia: "; cin >> dis;

        for (NodeMedicine *med = p->medical_record.stack_medicine.top; med!=NULL; med=med->next) {
            med->medicine = search_stack(sm, med->medicine.id)->medicine;
            med->medicine.price = med->medicine.price * (1-dis/100);
        }
    }
}
//Sắp xếp theo thứ tự tăng dần id khi in ra
void sort_stack_discount (StackMedicalRecord &smr) {
    for (NodeMedicalRecord *p=smr.top;p->next!=NULL;p=p->next) {
        for (NodeMedicalRecord *q=p->next;q!=NULL;q=q->next) {
            if (p->medical_record.discount > q->medical_record.discount) {
    ...
    if (f.is_open()) {
        while(!f.eof()) {
            ...
            for (i=0; i<n; i++) {
                ...
                getline(f,med.id);
                med = search_stack(sm, med.id)->medicine;
                med.price = med.price * (1-mr.discount/100);
...
void stack_qly_hsba (StackMedicalRecord &smr, StackPatient sp, StackMedicine sm) {
    ...
    do {
        ...
        case 10:
            ...
            break;
        case 11:
            fix_discount(smr,sp,sm);
            break;
        case 12:
            sort_stack_discount(smr);
            break;
        default:
            break;
        }
