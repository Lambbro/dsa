...
void output_stack (StackMedicine smed) {
    ...
    cout << string(TAB_WIDTH, ' ') << setw(10) << left << "ID"
		 ...
		 << setw(16) << right << "Giam gia"
		 << setw(13) << right << "Gia tien"
		 << setw(17) << right << "Lieu luong" << endl
		 << setw(17) << right << "Thanh tien" << endl;
...
//Exam
void fix_discount (StackMedicine &smed) {//Giam gia theo thuoc
    string id; cout << "Nhap id: "; cin.ignore(); getline(cin, id);
    if (search_stack(smed, id)) {
        float dis; cout << "Giam gia: "; cin >> dis;
        search_stack(smed,id)->medicine.discount = dis;
    } else {
        cout << "Khong tim thay benh nhan"; return;
    }
}

void sort_stack_by_discount(StackMedicine &smed) {
    for (NodeMedicine *p=smed.top; p->next!=NULL; p=p->next) {
        for (NodeMedicine *q=p->next; q!=NULL; q=q->next) {
            if (p->medicine.discount > q->medicine.discount) {
                Medicine m = p->medicine;
                p->medicine = q->medicine;
                q->medicine = m;
            }
        }
    }   
...
void sort_stack_by_discount_smr (StackMedicalRecord &smr) {
    for (NodeMedicalRecord *nmr=smr.top;nmr!=NULL;nmr=nmr->next) {
        sort_stack_by_discount(nmr->medical_record.stack_medicine);
    }
}
//Menu
void menu () {
...
void stack_qly_hsba (StackMedicalRecord &smr, StackPatient sp, StackMedicine sm) {
    ...
    do {
        ...
        case 10:
            ...
            break;
        case 11:
            cout << string(TAB_WIDTH, ' ') << "Da sap xep" << endl;
            sort_stack_by_discount_smr(smr);
            break;
...
void stack_qly_thuoc (StackMedicine &sm, StackMedicalRecord smr) {
    ...
    do {
        ...
        case 7:
            cout << string(TAB_WIDTH, ' ') << "Da sap xep" << endl;
            sort_stack_by_discount(sm);
            break;
        case 8:
            cout << string(TAB_WIDTH, ' ') << "Da sua" << endl;
            fix_discount(sm);
        default:
            break;
        }
