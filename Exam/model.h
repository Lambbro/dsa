...
struct Medicine { // Thuoc
	string id, name, uses;
	float price, medication_dosage, discount, total;//Them giam gia, tong tien
	...
	inline void input_from_keyboard () {
		...
		cout << string(TAB_WIDTH, ' ') << "Giam gia: "; cin >> discount;
		cout << string(TAB_WIDTH, ' ') << "Gia tien: "; cin >> price;
		cout << string(TAB_WIDTH, ' ') << "Lieu luong: "; cin >> medication_dosage;
        total = price*(1-discount/100)*medication_dosage;//Tong tien = gia * (1-giamgia) * lieu luong
	}
	inline void output_screen () {
		cout << string(TAB_WIDTH, ' ') << setw(10) << left << id
			...
			<< setw(10) << right << price <<"d/ml"
			<< setw(15) << right << medication_dosage << "ml" << endl
			<< setw(15) << right << total << "ml" << endl;
	}
};
